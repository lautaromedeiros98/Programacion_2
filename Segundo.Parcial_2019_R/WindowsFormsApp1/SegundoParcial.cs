...
namespace SP
{
    ...
    public partial class SegundoParcial: Form
    {
		...
		private static SqlCommand _comando2;
		private static SqlCommand _comando3;
        ...
        {
            MessageBox.Show("Apellido y nombre del alumno... Medeiros Lautaro");
			_conexion = new SqlConnection(@"Data Source=LALO-PC\SQLEXPRESS;Integrated Security=True");
        }
        ...
        private static string ObtenerListadoFrutas()
        {
			...
			_comando.CommandText = "SELECT * FROM [sp_lab_II].[dbo].[frutas]";
			...
			try
			{
				...
				while (_reader.Read())
				{
					retorno.AppendLine(_reader["id"].ToString()  +"-"+ _reader["nombre"].ToString() + "-" + _reader["peso"].ToString() + "-" + _reader["precio"].ToString());
				}
			}
			catch (Exception e)
			{
        ...
        {
			SegundoParcial._comando = new SqlCommand();
			SegundoParcial._comando.Connection = _conexion;
			SegundoParcial._comando.CommandType = CommandType.Text;
			SegundoParcial._comando.CommandText = "INSERT INTO sp_lab_II.dbo.frutas(nombre,peso,precio) values('" + frm._manzana.Nombre + "','" + frm._manzana.Peso.ToString() + "','" + 4.ToString() + "')";
			try
			{
				_conexion.Open();
				if (_comando.ExecuteNonQuery() > 0)
				{
					SegundoParcial._comando.CommandText = "INSERT INTO sp_lab_II.dbo.frutas(nombre,peso,precio) values('" + frm._banana.Nombre + "','" + frm._banana.Peso.ToString() + "','" + 2.ToString() + "')";
					if(_comando.ExecuteNonQuery() > 0)
					{
						SegundoParcial._comando.CommandText = "INSERT INTO sp_lab_II.dbo.frutas(nombre,peso,precio) values('" + frm._durazno.Nombre.ToString() + "','" + frm._durazno.Peso.ToString() + "','" + 1.ToString() + "')";
						if(_comando.ExecuteNonQuery() > 0)
						{
							_conexion.Close();
							return true;
